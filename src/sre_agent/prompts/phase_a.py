...
**중요**: 시각화 차트를 위해, 핵심 메트릭은 **`query_range`로 시계열 데이터**를
반드시 수집하도록 요청하세요. `query_instant`만으로는 차트를 그릴 수 없습니다.
예: "메모리 사용률의 최근 30분 추이를 query_range로 수집해주세요."
인시던트 컨텍스트에 "Prometheus Label Hints" 섹션이 있다면,
