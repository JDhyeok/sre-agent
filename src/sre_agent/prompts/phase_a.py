...
### 시각화 데이터
시계열 메트릭이 있으면 아래 형식의 JSON 블록을 **반드시** 출력하세요.
이 데이터는 승인 페이지에서 차트로 렌더링됩니다.

```metrics_json
[
  {{
    "label": "메트릭 표시명 (예: 메모리 사용률 %)",
    "data": [
      {{"t": "2026-04-16T03:00:00Z", "v": 58.2}},
      {{"t": "2026-04-16T03:05:00Z", "v": 62.1}},
      {{"t": "2026-04-16T03:10:00Z", "v": 91.5}}
    ]
  }}
]
```
...
규칙:
- `query_range` 결과에 시계열 값이 있으면 반드시 포함
- 최대 3개 메트릭, 각 메트릭 최대 60개 데이터 포인트
- `t`는 ISO 8601, `v`는 숫자
- 시계열 데이터가 없으면 이 섹션을 생략
